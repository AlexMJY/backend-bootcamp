...
public class Basic {
/* 클래스 정의 --------------------------------------
 * [접근제어자] [클래스 종류] class 클래스이름 [extends 부모클래스이름]
 *                                  [implements 인터페이스이름] {
 *  public     abstract
 *  package    final
 *  	
 ...
 * 
 * 클래스의 상속
 * - 부모 클래스super class와 자식 클래스 사이의 관계
 * - 부모 클래스의 필드와 메서드를 자식 클래스에서 상속 받음
 * - 단일 클래스 상속만 가능
 * - 생성자 상속 X
 *   - 하위 클래스의 객체가 생성될 때
 *     상위 클래스의 생성자super()가 자동으로 호출됨
 *     - 상위 클래스의 데이터 필드 초기화
 ...
 *     - 부모의 클래스의 메소드에 접근할 때는 super 키워드를 이용
 *   
 * - 상속 불가 클래스는 선언 시 final 명시
 * - 자바의 최상위 클래스 java.lang.Object는
 *   묵시적으로 자동 상속
 * 
...
/* 추상화abstraction --------------------------
 * - 구체적인 개념으로부터 공통된 부분들만 추려내려 일반화
 *   - 일반적으로 사용할 수 있는 단계가 아닌 미완성의 틀 개념
 *   - 클래스 및 메서드에 적용 
 *   
 * 클래스의 추상화
 ...
 * 직접 예외 처리
 * - 던져진 예외 객체를 잡아서 처리
 * - try / catch 구문 또는 try / catch / finally 구문 사용
 * 
 * 간접 예외 처리
