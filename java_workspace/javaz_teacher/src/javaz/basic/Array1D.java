...
//		users = { "Ann", "Ben", "Ken" };
//				 이미 선언된 배열의 경우는 위처럼은 X
//		                            아래처럼은 O
		users = new String[]{ "Kim", "Lee", "Han" };
		
		int[] scores;	//정수 데이터들을 저장하는 scores 배열 선언
		char grades[];	//문자 데이터들을 저장하는 grades 배열 선언
		
		scores = new int[3]; //scores 배열 변수를 3개 크기를 갖도록 생성
		grades = new char[3];//grades   "
		
		boolean[] pass = new boolean[3];	//논리값 데이터 3개를 저장하는 pass 배열 변수 선언 및 생성
		
		scores[0] = 99;	//scores 배열의 첫번째는 99,
		scores[2] = 77;	//세번째는 77로 초기화
//		scores[3] = 77; //배열의 범위를 넘어가는 인덱스 지정 예외
		
		//scores 배열의 값을 화면에 출력
		System.out.println(scores);	//배열의 주소 출력
		System.out.println(scores[0]);
		System.out.println(scores[1]);
		System.out.println(scores[2]);
		
		System.out.println();
		System.out.println("length 필드 이용");
