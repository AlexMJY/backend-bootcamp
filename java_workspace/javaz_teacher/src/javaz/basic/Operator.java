...
//		System.out.println(divide / 0);		//0으로 나누기 X
		System.out.println(divide / 0.0);	//0.0으로  "
		...
		//--	변수의 값을 1 감소
		int year = 2024;
		System.out.println("last year : "    + --year); //먼저 1감소
		System.out.println("this year : "    + ++year); //먼저 1증가
		System.out.println("still year : "   + year--);//나중에 1감소
		System.out.println("back to year : " + year++);//나중에 1증가
		...
		System.out.println();
		//비교 연산자 ------------------------------	
		//비교 결과 true 또는 false반환
		//>, <, >=, <= 	 boolean을 제외한 기본형 변수 값 비교
		//		==, !=	 기본형은 값 비교, 
		//			 	 참조형은 주소 비교
		//				 모든 자료형에 사용 가능 
		//				 단, 문자열 비교 X
		...
		Operator o = new Operator(); //현재 클래스의 인스턴스 o 생성
		System.out.println( o instanceof Operator );//객체 o가 Operator 클래스의 인스턴스이면 true를 출력 
		System.out.println( o instanceof Object );//참조변수 o가 Object   "       "       "    " 
//		System.out.println( o instanceof MuInCar );//참조변수 o가 MuInCar  "       "       "    " 
...
//		단, 접근제어자에 의해 사용 가능 여부 결정
		...
		input = 333;
		result = (input%3) == 0; //input이 3의 배수이면 true 아니면 false 
		result = (input%3) != 0;//input이 3의 배수가 아니면  true 아니면 false
		
		//input이 100 미만의 3의 배수?
		result = (input < 100) && (input%3 == 0);  
		
		//input이 100 이상이거나 3의 배수?
		result = (input >= 100) || (input%3 == 0);  
