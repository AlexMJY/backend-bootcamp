...
public class CarMain {
	...
	public static void main(String[] args) {
		...
		yellowCar.door = 4;
//		yellowCar.key = false;
		//key 필드가 private으로 선언되었기 때문에 사용 X
		yellowCar.setKey(false);  //이렇게는 가능
		
		/////////////////////////
		//Car 클래스의 객체 redCar 생성
		Car redCar = new Car();
		redCar.color = "빨강";
		redCar.maker = "현대";
		redCar.door = 2;
...
//		System.out.println("키 상태 : " + yellowCar.key);
		...
		//2.key 필드의 값을 반환하는 메서드를 호출하여
		//  반환되는 값을 keyStatus 변수에 저장하고
		boolean keyStatus = yellowCar.getKey();		
		
		//3.키 상태를 화면에 출력
		System.out.println("키 상태 : " + keyStatus);
...
//		System.out.println("키 상태 : " + redCar.key);
		...
		ec.color = "silver"; //ec의 색을 silver로 지정
		ec.maker = "KIA";	 //ec의 maker는 KIA로 지정
		ec.door = 4;		 //ec의 door는 4개로 지정
		ec.setKey(true);  	 //ec의 key를 true로 지정
		
		//생성된 객체를 문자열로 출력
		System.out.println(ec);
		
		//ec의 현재 배터리 용량 출력 
		System.out.println("현재 배터리 용량 : " +
						   ec.getBattery());
		...
		ElectricCar elecCar 
			= new ElectricCar(10000, tire1, tire2, tire3, tire4);
		
		System.out.println();
		System.out.println("- 미쉐린 타이어를 장착한 전기툭툭이 만들기 -");
		//배터리 용량은 5000	
		tire1 = new Tire("미쉐린 타이어", "앞");
		ElectricCar elecTuk 
			= new ElectricCar(10000, tire1, tire3, tire4);
